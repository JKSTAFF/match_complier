...
if [ `grep -c  "CONFIG_LUCI_LANG_zh_Hans=y" ".config"` -ne '0' ]; then 
    ...
    echo "Sinification due to luci language setting"
fi
...
echo "Set 'toor' as the root password."
...
if [ `grep -c  "CONFIG_PACKAGE_kmod-tcp-bbr=y" ".config"` -ne '0' ]; then 
    echo "net.core.default_qdisc=fq" >> package/base-files/files/etc/sysctl.conf && echo "net.ipv4.tcp_congestion_control=bbr" >> package/base-files/files/etc/sysctl.conf
